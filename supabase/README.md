...
Run the three files **in order**, in the SQL editor or with `psql`:

```bash
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f supabase/migrations/0001_schema.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f supabase/migrations/0002_functions.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f supabase/seed/0003_demo_data.sql   # optional
```

`0001` drops the tables it is about to create, so re-running it during
development is safe — and destructive. The seed truncates the
transactional tables before it writes, so it is also safe to re-run.

Do **not** use the old `data_base.sql` at the repository root. It is a full
`pg_dump` including `CREATE SCHEMA auth`, `storage`, `realtime` and the
rest, which collide with the schemas Supabase manages for you.
...
holding the service-role key, and those routes resolve the caller's role
from `employees` before they do anything — the browser only ever holds the
anon key and only reads. RLS would be a second line of defence, not the
